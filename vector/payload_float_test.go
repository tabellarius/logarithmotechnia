...
func TestFloatPayload_Adjust(t *testing.T) {
	...
	testData := []struct {
		...
		size       int
		outPayload *floatPayload
	}{
		{
			inPayload:  payload5,
			name:       "same",
			size:       5,
			outPayload: FloatPayload([]float64{1 + 0i, 2.5, 5.5, 0, -2}, nil).(*floatPayload),
		},
		{
			inPayload:  payload5,
			name:       "lesser",
			size:       3,
			outPayload: FloatPayload([]float64{1 + 0i, 2.5, 5.5}, nil).(*floatPayload),
		},
		{
			inPayload: payload3,
			name:      "bigger",
			size:      10,
			outPayload: FloatPayload([]float64{1 + 0i, 2.5, math.NaN(), 1 + 0i, 2.5, math.NaN(),
	...
	for _, data := range testData {
		t.Run(data.name, func(t *testing.T) {
			...
			if !util.EqualFloatArrays(outPayload.data, data.outPayload.data) {
				t.Error(fmt.Sprintf("Output data (%v) does not match expected (%v)",
...
func TestFloatPayload_Traverse(t *testing.T) {
	payload := FloatPayload([]float64{1, 2, 3, 4, 5}, []bool{false, false, true, false, true})

	cntFull, cntCompact, cntBrief := 0, 0, 0
	trF := func(int, float64, bool) { cntFull++ }
	trC := func(float64, bool) { cntCompact++ }
	trB := func(float64) { cntBrief++ }
