...
func TestIntegerPayload_Adjust(t *testing.T) {
	...
	testData := []struct {
		...
		size       int
		outPayload *integerPayload
	}{
		{
			inPayload:  payload5,
			name:       "same",
			size:       5,
			outPayload: IntegerPayload([]int{1, 2, 3, 4, 5}, nil).(*integerPayload),
		},
		{
			inPayload:  payload5,
			name:       "lesser",
			size:       3,
			outPayload: IntegerPayload([]int{1, 2, 3}, nil).(*integerPayload),
		},
		{
			inPayload: payload3,
			name:      "bigger",
			size:      10,
			outPayload: IntegerPayload([]int{1, 2, 0, 1, 2, 0, 1, 2, 0, 1},
...
func TestIntegerPayload_Pick(t *testing.T) {
...
func TestIntegerPayload_ApplyTo(t *testing.T) {
	srcPayload := IntegerPayload([]int{1, 2, 3, 4, 5}, []bool{false, true, false, true, false})
...
func TestIntegerPayload_Traverse(t *testing.T) {
	payload := IntegerPayload([]int{1, 2, 3, 4, 5}, []bool{false, false, true, false, true})

	cntFull, cntCompact, cntBrief := 0, 0, 0
	trF := func(int, int, bool) { cntFull++ }
	trC := func(int, bool) { cntCompact++ }
	trB := func(int) { cntBrief++ }
